-- AquaVault Capital - Dynamic Investment System Update (SAFE VERSION)
-- This script safely adds the new dynamic category and duration system
-- Run this AFTER importing the main schema.sql

-- First, let's drop the problematic investment_categories table if it exists
-- and recreate it with the correct structure
DROP TABLE IF EXISTS `investment_categories`;

-- Create investment_categories table with correct structure
...
-- Drop and recreate investment_durations table if it exists
DROP TABLE IF EXISTS `investment_durations`;
...
-- Add category_id column to investment_plans table (only if it doesn't exist)
SET @sql = (SELECT IF(
...
-- Add category_id column to user_investments table (only if it doesn't exist)
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
...
-- Add duration_id column to user_investments table (only if it doesn't exist)
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'user_investments' 
...
-- Add foreign key for category_id in investment_plans (only if it doesn't exist)
SET @sql = (SELECT IF(
...
-- Add foreign key for category_id in user_investments (only if it doesn't exist)
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'user_investments' 
...
-- Add foreign key for duration_id in user_investments (only if it doesn't exist)
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'user_investments' 
